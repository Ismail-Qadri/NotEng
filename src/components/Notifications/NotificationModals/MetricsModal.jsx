...
const MetricsModal = ({ onSave, metric, onCancel }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDimensions(Array.isArray(dimensionsRes.data) ? dimensionsRes.data : []);
      } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-8 mt-10 transform transition-transform scale-100 max-h-[65vh] flex flex-col overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={e => setExecuteSqlMain(e.target.value)}
          ...
          <div>
            ...
            <textarea
              ...
              disabled={loading}
              rows={4}
              placeholder={t("optionalConditionalSqlQuery") || "Optional conditional SQL query"}
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              disabled={loading}
            >
              <option value="">{t("selectUseCase") || "Select Use Case"}</option>
              {useCases.map(uc => (
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2">
              {t("dimension")} ({t("optional") || "Optional"})
            </label>
            <select
              value={dimensionId}
              ...
              disabled={loading}
            >
              <option value="">{t("selectDimension") || "Select Dimension"}</option>
              {dimensions.map(d => (
