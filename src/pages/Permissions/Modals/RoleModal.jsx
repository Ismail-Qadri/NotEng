...
const RoleModal = ({
  ...
  const jwt = localStorage.getItem("userId"); // or your actual JWT/localStorage key
      ...
      if (role) {
        try {
          ...
          if (roleData.resources) {
            roleData.resources.forEach(
              ({ resource_id, permissions: permIds }) => {
                const resource = resources.find(
                  (r) => r.id.toString() === resource_id
                );
                if (resource) {
  ...
  const handleToggle = (resource, permission) => {
    ...
    setSelected((prev) => {
      ...
      if (already) {
        newSelected[resourceName] = newSelected[resourceName].filter(
          (p) => p !== permissionName
        );
      } else {
        newSelected[resourceName] = [
          ...(newSelected[resourceName] || []),
          permissionName,
        ];
      }
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              id="description"
              placeholder={
                t("descriptionPlaceholder") || "Enter role description"
              }
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-col gap-4 mt-2">
              {Object.entries(groupedResources).map(([category, resArr]) => (
                <div key={category}>
                  <div className="font-bold text-base text-gray-600 mb-2">
                    {category}
                  </div>
                  {resArr.map((resource) => (
                    <div
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {permissions.map((permission) => {
                          const Icon =
                            permissionIconsById[permission.id] || Eye;
                          const isActive = selected[resource.name]?.includes(
                            permission.name
                          );
                          return (
                            <button
